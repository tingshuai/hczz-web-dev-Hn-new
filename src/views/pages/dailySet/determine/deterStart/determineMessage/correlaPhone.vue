		...
		<div class="wholeContent padding-left-20 padding-right-20 clearfix">
			<Form ref="formDynamic" :model="formDynamic" inline class="searchForm">
				<FormItem label="通讯号码：" :label-width="84">
					<Input type="text" clearable :maxlength="11" @on-keyup="sjhmChange" v-model.trim="formDynamic.sjhm" placeholder="请输入通讯号码"></Input>
				</FormItem>
			...
			<addModalPhones :show="show" :sqid="sqid" @update="update" @closeModal="closeModal" :phoneId="phoneId"></addModalPhones>
			<!--删除二次确认-->
...
<script>
	...
	export default {
		...
		methods: {
			...
			// input 输入框，只能输入数字
			sjhmChange() {
				this.formDynamic.sjhm = this.formDynamic.sjhm.replace(/[^\d]/g,'')
			},
			pageChange(current) {
