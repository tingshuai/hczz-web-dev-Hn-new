	...
	<div>
		<div class="clearfix tableStyle">
			<table border="1" cellspacing="0" v-if="rwly==0">
				<tr>
					<th>事件名称</th>
					<th colspan="3" :title="detailObj.ajmc">{{detailObj.ajmc}}</th>
				</tr>
				<tr>
					<th>事件类型</th>
				...
				<tr>
					<th>任务附件</th>
					<th colspan="3">
						<a v-for='(item,index) in columnsData' @click="handlePrev(item)" :key="index">
				...
				<tr>
					<th>手续附件(包含驳回材料)</th>
					<th colspan="3">
							<a v-for='(item,index) in columnsDataone' @click="handlePrev(item)" :key="index">
			...
			<table border="1" cellspacing="0" v-else>
				<tr>
					...
					<th>事件名称</th>
					<th :title="detailObj.ajmc" class="text-overflow">{{detailObj.ajmc}}</th>
				</tr>
				<tr>
					<th>事件类型</th>
					<th>{{detailObj.jqajlx}}</th>
				...
				<tr>
					<th>事件描述</th>
					<th colspan="3" :title="detailObj.jqajzs" class="text-overflow">{{detailObj.jqajzs}}</th>
				</tr>
				<tr>
					<th>事件案发地点</th>
					<th colspan="3" :title="detailObj.afdz" class="text-overflow">{{detailObj.afdz}}</th>
				</tr>
				<tr>
					<th>申请人</th>
					<th>{{sqxxObj.sqr}}</th>
				...
				<tr>
					...
					<th colspan="3">
						<a v-for='(item,index) in columnsData' @click="handlePrev(item)" :key="index">
							<span>
								<svg-icon name='picIcon'></svg-icon>
