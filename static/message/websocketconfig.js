// eslint-disable-line
var websocketurl="ws://"+"192.168.0.71"+":2048/ws";   //ws://{ip}:{端口}/{java后端websocket配置的上下文}
...
function createWebSocket(url,callbak) {
    try {
        ...
        if (window.WebSocket) {
            ...
            //当浏览器不支持websocket时 降级为http模式 eslint-disable-line
            var isClose =false;
...
function reconnect(url,callbak) {
    ...
    reconnectflag = true;
    //没连接上会一直重连，设置延迟避免请求过多 eslint-disable-line
    setTimeout(function () {
