 ...
 *  */
const Watermark = function (node, options) {
...
Watermark.prototype = {
  ...
  _init: function (node, options) {
    ...
    node.classList.add('co-');
  },
  /* 自动调整每个水印间距 使其可以填充整个页面 */
  _resizeSpace: function () {
    this.pageWidth = Math.max(this.node.scrollWidth, this.node.clientWidth);
    this.pageHeight = 1 + Math.max(this.node.scrollHeight, this.node.clientHeight);
    // 计算旋转后的元素所占宽度和高度
  ...
  _fillContent: function () {
    ...
    this.node.appendChild(this.markContainer);
  },
  /* 构造每个水印节点 */
  ...
  refresh: function () {
    ...
    this._init(this.node, this.options);
    this._resizeSpace();
    this._fillContent();
  },
  /* 显示水印图层 */
